...
whenExists('#mega-menu-item-11235 > ul, #mega-menu-item-11235 > a', function () {
	...
	Object.values(da).forEach((el, i) => {
		Object.entries(el).forEach((element, j) => {
			if (j == 0) {
				$(`<li><div class="da_sub-section">${element[0]}</div><ul style="display: none" list-item="${i+1}"></ul></li>`).appendTo('.da_menu');	
				return;
			}
	...
	if (window.innerWidth < 681) {
		...
		$('.da_menu > li').click(function() {
			...
			$(this).find('.da_sub-section').toggleClass('da_active-sub-section');
		});
	}
