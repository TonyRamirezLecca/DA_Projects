		...
		whenExists('#product-options-wrapper > .last', function () {
			...
			for (var i = 0; i < newTitles.length; i++) {
				...
				for (var j = 0; j < newContentSection[i].length; j++) {
					jQuery(`<div class="newContent"><input class="with-gap" name="group${i}" type="radio">${newContentSection[i][j]}</input></div>`).appendTo('.sectionFlexCtn');
				}
			}
			...
			document.querySelectorAll('.sectionFlexCtn > .newContent:first-child').forEach((el, i, arr) => {
